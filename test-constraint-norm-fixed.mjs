...
function normalizeConstraintDefinition(def) {
  try {
    ...
    // Strip PostgreSQL's explicit type casts (e.g., (0)::numeric)
...
if (normLive === normDesired) {
  console.log('✓ Fix successful! Type casts are now stripped during normalization.');
} else {
  console.log('✗ Still not matching. Further investigation needed.');
}
